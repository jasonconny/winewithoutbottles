...
- Routing uses `react-router-dom` v7 (`createBrowserRouter` + `RouterProvider` in `src/index.tsx`); routes are defined in `src/router.tsx`. `/` and `/placeholder` → `Placeholder` (hero), `/builder` → the unlinked builder easter egg, `*` → redirect home. The router is the single switch point: repoint `/` to the real app when ready. `public/_redirects` provides SPA fallback for static hosting.

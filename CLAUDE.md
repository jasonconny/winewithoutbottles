...
- **Data pipeline (offline generation)**: the "machine" is the framework-free `src/wwob/` module (`titleToRgb`, `durationToWidth`, `buildStripes`, `svgMarkup`). Show data is hand-authored in `data/shows/<year>/<id>.json` (id = `YYYY-MM-DD`, organized into per-year subdirectories); `npm run generate` (`generator/generate.ts`, run via `tsx`, type-checked by `tsconfig.generator.json`, reads `data/shows` recursively) writes three kinds of output: per-show `public/shows/<id>.svg`, per-show `public/shows/<id>.json` (full `ShowDetail` incl. the setlist), and a light, songs-free index `src/data/shows.generated.ts` (`ShowSummary[]`) the app bundles. **The split matters for scale**: the bundled index stays small (~300 bytes/show) so the JS bundle is roughly flat as shows grow, while the heavy `songs` data is fetched per-show on demand (see Routing — the `/shows/:id` loader fetches `public/shows/<id>.json`; Gallery/Placeholder use only the in-bundle index). Pass filters (show id, filename, path, or quoted glob — e.g. `npm run generate 1977-05-08`, `npm run generate '1977-*'`) to rewrite only matching shows' per-show artifacts (SVG + detail JSON); the index is always rebuilt from every show so it stays complete and sorted. **Generated files are committed** (so CI needn't run the generator) and are eslint/prettier-ignored. The hand-authored data in `data/shows/<year>/<id>.json` is the **source of truth** and is freely editable for corrections (edit data → `npm run generate <id>` → commit; the `public/shows/<id>.svg` diff is the reviewable record). Reconstructing a legacy show is a bootstrap from Jason's original 2013 SVGs via `npx tsx generator/verify.ts <id>`: extract mode (no data file yet) dumps each stripe's target colour + width; verify mode (data file present) checks the authored setlist against them (durations from stripe widths, titles color-verified — a wrong title fails the colour check). `verify.ts` is a permanent dev tool, but the legacy SVGs it reads are **transient working inputs**: drop the masters into `tests/fixtures/legacy/<id>.svg` for the batch you're reconstructing (gitignored — Jason keeps the masters off-repo). The originals encode their own mistakes, which we correct over time (logged in `data/CORRECTIONS.md`), so there is **no fidelity checksum**. The committed guard is `tests/data-validity.test.ts`, which keeps the authored data well-formed (id matches filename, valid `m:ss` durations, non-empty setlists, no duplicate ids) and the generated manifest's id set in sync with the data files.
...
- Routing uses `react-router-dom` v7 (`createBrowserRouter` + `RouterProvider` in `src/index.tsx`); routes are defined in `src/router.tsx`. `/` and `/placeholder` → `Placeholder` (random-art hero); `/shows` + `/shows/:id` → the (hidden) Gallery/Show reader (the `/shows/:id` route has a `loader` that fetches `public/shows/<id>.json` for the full detail; a missing id → null → "not found"); `/about` → About; `/builder` → the unlinked builder easter egg; `*` → redirect home. The router is the single switch point: repoint `/` to the real app when ready. **The future homepage will be identical to the current `Placeholder` (random striped piece + watermark logotype) except for added navigation into the rest of the project — the flip = repoint `/` + add nav, nothing else.**

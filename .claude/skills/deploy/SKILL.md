...
soft-404 to crawlers. **Deferred to the home flip** (harmless while nothing
links into the reader): put a small Worker in front of the fallback (`main` +
...
so pair it with a test that keeps it in sync with `src/router.tsx`. This is a
separate staging Worker (`*.workers.dev`); the live temp site (`wwob-tmp` repo
at `/Users/jasonconny/Sites/wwob-tmp`) still holds winewithoutbottles.com
until the custom domain is moved over manually.
